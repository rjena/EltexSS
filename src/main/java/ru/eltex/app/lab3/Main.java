...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("\nTrying to find new random UUID in cart: " + orders.getOrders().get(0).getCart()
                    .containsID(UUID.randomUUID()));
            System.out.println("Trying to find not random UUID in cart: " + orders.getOrders().get(0).getCart()
                    .containsID(((Electronic) orders.getOrders().get(0).getCart().getCartItems().get(0)).getID()));
        } catch (Exception e) {
